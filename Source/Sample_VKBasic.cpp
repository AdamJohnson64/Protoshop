...
public:
  Sample_VKBasic(std::shared_ptr<DXGISwapChain> swapchain,
      ...
      : m_pSwapChain(swapchain), m_pDeviceVK(pDeviceVK),
        m_pDeviceD3D12(pDeviceD3D12) {}
  void Render() override {
    // We can't directly use the image from DXGI, probably because it's a D3D9Ex
    // image. Create a shared D3D12 image to use as our render target and copy
    // it back later.
    CComPtr<ID3D12Resource1> pD3D12Backbuffer;
    {
      ...
      descClear.DepthStencil.Depth = 1.0f;
      {
        ...
        TRYD3D(m_pDeviceD3D12->m_pDevice->CreateCommittedResource1(
            ...
            __uuidof(ID3D12Resource1), (void **)&pD3D12Backbuffer.p));
    ...
    TRYD3D(m_pDeviceD3D12->m_pDevice->CreateSharedHandle(
        pD3D12Backbuffer, nullptr, GENERIC_ALL, nullptr,
        (HANDLE *)&hD3D12Backbuffer));
    ...
    vk::UniqueImage vkImage;
    {
      ...
      vkImage = m_pDeviceVK->m_vkDevice->createImageUnique(
    ...
    vk::UniqueDeviceMemory vkDeviceMemory;
    {
      vk::MemoryRequirements mr =
          m_pDeviceVK->m_vkDevice->getImageMemoryRequirements(vkImage.get());
      vk::StructureChain<vk::MemoryAllocateInfo,
          ...
          sc = {
              ...
              vk::ImportMemoryWin32HandleInfoKHR(
                  vk::ExternalMemoryHandleTypeFlagBits::eD3D12Resource,
                  hD3D12Backbuffer.get())};
      vkDeviceMemory = m_pDeviceVK->m_vkDevice->allocateMemoryUnique(
          sc.get<vk::MemoryAllocateInfo>());
    }

    // Connect the image with its memory.
    m_pDeviceVK->m_vkDevice->bindImageMemory(vkImage.get(),
                                             vkDeviceMemory.get(), 0);
    ...
    vk::UniqueFramebuffer vkFramebuffer;
    {
      ...
      vk::FramebufferCreateInfo cfb({}, m_pDeviceVK->m_vkRenderPass.get(),
    ...
    VK_Run_Synchronously(
        m_pDeviceVK.get(), [&](vk::CommandBuffer m_vkCommandBuffer) {
          {
            std::vector<vk::ImageMemoryBarrier> imb = {vk::ImageMemoryBarrier(
                (vk::AccessFlagBits)0, (vk::AccessFlagBits)0,
                vk::ImageLayout::eUndefined, vk::ImageLayout::eGeneral,
                VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, vkImage.get(),
