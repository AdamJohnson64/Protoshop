...
public:
    OpenGLDeviceImpl()
    {
        ...
        int nPixelFormat = ChoosePixelFormat(m_hDeviceContext, &pixelFormatDescriptor);
        if (0 == nPixelFormat) throw FALSE;
        // Use this format (this may only be set ONCE).
        if (TRUE != SetPixelFormat(m_hDeviceContext, nPixelFormat, &pixelFormatDescriptor)) throw FALSE;
        // Create a GL context for this window.
        m_hOpenGLResourceContext = wglCreateContext(m_hDeviceContext);
        if (nullptr == m_hOpenGLResourceContext) throw FALSE;
    }
    HGLRC GetHLGRC() override
    {
        return m_hOpenGLResourceContext;
    }
private:
