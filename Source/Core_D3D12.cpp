...
Direct3D12Device::Direct3D12Device() {
  ////////////////////////////////////////////////////////////////////////////////
  // Enable D3D12 Debugging (note: DXGI may fail when tearing down the
  // swapchain).
  {
    CComPtr<ID3D12Debug3> pD3D12Debug;
    TRYD3D(
        D3D12GetDebugInterface(__uuidof(ID3D12Debug3), (void **)&pD3D12Debug));
    // By all means turn this on when you're debugging, but this will crash on
    // exit due to the swap chain.
  ...
  // Comment out this line to use a real hardware GPU (not the WARP adapter).
  // TRYD3D(pDXGIFactory7->EnumWarpAdapter(__uuidof(IDXGIAdapter4),
  // (void**)&pDXGIAdapter4Warp));
  ////////////////////////////////////////////////////////////////////////////////
  // Create the Direct3D 12 device.
  TRYD3D(D3D12CreateDevice(pDXGIAdapter4Warp, D3D_FEATURE_LEVEL_12_1,
                           __uuidof(ID3D12Device6), (void **)&m_pDevice));
  ...
  // Create our primary command queue.
  {
    ...
    TRYD3D(m_pDevice->CreateCommandQueue(
        &descQueue, __uuidof(ID3D12CommandQueue), (void **)&m_pCommandQueue));
  ...
  // Create a descriptor heap for the primary RTV.
  {
    ...
    TRYD3D(m_pDevice->CreateDescriptorHeap(&descDescriptorHeap,
                                           __uuidof(ID3D12DescriptorHeap),
                                           (void **)&m_pDescriptorHeapRTV));
