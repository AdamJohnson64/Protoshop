...
void ParametricUVToMesh::copyIndices(void* to, uint32_t stride)
{
    for (int32_t v = 0; v < m_stepsInV; ++v)
    {
        for (int32_t u = 0; u < m_stepsInU; ++u)
        {
            *(reinterpret_cast<uint32_t*>(to)) = (u + 0) + (v + 0) * (m_stepsInU + 1);
            to = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(to) + stride);
            *(reinterpret_cast<uint32_t*>(to)) = (u + 0) + (v + 1) * (m_stepsInU + 1);
            to = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(to) + stride);
            *(reinterpret_cast<uint32_t*>(to)) = (u + 1) + (v + 1) * (m_stepsInU + 1);
            to = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(to) + stride);
            *(reinterpret_cast<uint32_t*>(to)) = (u + 0) + (v + 0) * (m_stepsInU + 1);
            to = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(to) + stride);
            *(reinterpret_cast<uint32_t*>(to)) = (u + 1) + (v + 1) * (m_stepsInU + 1);
            to = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(to) + stride);
            *(reinterpret_cast<uint32_t*>(to)) = (u + 1) + (v + 0) * (m_stepsInU + 1);
            to = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(to) + stride);
        }
    }
}