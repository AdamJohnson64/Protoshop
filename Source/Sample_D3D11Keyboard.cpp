...
float4 mainPS(Vertex vin) : SV_Target
{
  ...
  std::shared_ptr<FontASCII> theFont = CreateFreeTypeFont();
  CComPtr<ID3D11ShaderResourceView> srvFont =
      D3D11_Create_SRV(device->GetID3D11DeviceContext(), theFont->Atlas.get());
  ////////////////////////////////////////////////////////////////////////////////
  // Create a vertex buffer.
  std::shared_ptr<CComPtr<ID3D11Buffer>> bufferVertex =
      std::make_shared<CComPtr<ID3D11Buffer>>();
  std::shared_ptr<int> vertexCount = std::make_shared<int>(0);
  {
  ...
  request.Render = [=](const SampleResourcesD3D11 &sampleResources) {
    ...
    device->GetID3D11DeviceContext()->Draw(*vertexCount.get(), 0);
    device->GetID3D11DeviceContext()->ClearState();
